        ...
        if contains(file_list(i).name, 'capture')
            ...
            for j = 1 : length(capture_data_list)
                if contains(capture_data_list(j).name, '.hdr')
                    ...
                    if (not (contains(capture_data_list(j).name,'DARKREF_'))) && (~contains(capture_data_list(j).name,'WHITEREF_') && (~contains(capture_data_list(j).name,'WHITEDARKREF_')))                       
                        hdr_file = strcat(str_temp, '\',capture_data_list(j).name);                         
                    elseif (contains(capture_data_list(j).name,'DARKREF_') && (~contains(capture_data_list(j).name,'WHITEDARKREF_')))                        
