...
mahal_Dist = zeros(total_Pixels, total_Pixels);

for pixelPosA = 1:total_Pixels
    for pixelPosB = 1:total_Pixels
        diff =  (vectorizedInputHsi(pixelPosA, :) - vectorizedInputHsi(pixelPosB, :));
        diff_T = diff';
        mahal_Dist(pixelPosA, pixelPosB) = sqrt(( diff / covar) * diff_T);
    end
end

% This should be n x n matrix. Need correction.
[val, id] = max(mahal_Dist,[], 1);
% [val, id] = max(w_s,[], 1);
