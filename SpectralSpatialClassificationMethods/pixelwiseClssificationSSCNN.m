...
%
% This script should run image by image. It doesn't work as a batch
% classification. 
% All the classes should be identified before classification. Then run the
% band selection as well
...
%%
imageFileName = 'REFLECTANCE_2020-04-27_004.dat';
...
for hx = 1:X_diff
    for hy = 1:Y_diff
        sampleCube(hx, hy, :) = rotatedHsi( topLeft(1,1) + hy, topLeft(1,2) + hx, :);
        copyRGB( topLeft(1,1) + hy, topLeft(1,2) + hx, :) = [0 0 255];
    end
end
...
numTrain = totalSamples * 0.8;
numValid = totalSamples - numTrain;

% cnnTrainData = zeros(ssKernel, ssKernel, bandsCount, numTrain);
% cnnTrainLabels = zeros(1, numTrain, 'uint8');

cnnTrainData = augData(:,:,:, 1:numTrain);
cnnValidData = augData(:,:,:, numTrain +1:totalSamples);

cnnTrainLabels = augLabels(1, 1:numTrain);
cnnValidLabels = augLabels(1, numTrain +1:totalSamples);

% cnnValidData = zeros(ssKernel, ssKernel, bandsCount, numValid);
% cnnValidLabels = zeros(1, numValid, 'uint8');

augTrainLabelsCat = categorical(cnnTrainLabels);

augValidLabelsCat = categorical(cnnValidLabels);



%% Neural Network 

fcLayerNeurons = length(categories(augTrainLabelsCat));

layers = [
    imageInputLayer([5 5 bandsCount],"Name","imageinput")
    convolution2dLayer([3 3],16,"Name","conv_1","Padding","same")
    reluLayer("Name","relu_1")
    convolution2dLayer([3 3],16,"Name","conv_2","Padding","same")
    reluLayer("Name","relu_2")
    fullyConnectedLayer(fcLayerNeurons,"Name","fc")
...
opts = trainingOptions('adam', ...
    ...
    'ValidationData',{cnnValidData, augValidLabelsCat}, ...
    'Shuffle', 'every-epoch', ...
    'ExecutionEnvironment','gpu'); %Optimise using stochastic gradient descent with momentum

deep_net_25 = trainNetwork(cnnTrainData, augTrainLabelsCat, layers, opts);
...
tempColors = zeros(1, 3, 'uint8');
listCategories = categories(augTrainLabelsCat);

for n = 1: length(listCategories)
    colorId = str2num(cell2mat(listCategories(n, 1)));
    tempColors(n,:) = Get_Label_Color(colorId + 1);
end
...
for i = 1:w
    for j = 1:h
        ...
        if (colorIdx == 0)
            resultHsi_vis(i, j, :) = tempColors(colorIdx + 1, :);
        else
            resultHsi_vis(i, j, :) = tempColors(colorIdx, :);
        end
    end
end
